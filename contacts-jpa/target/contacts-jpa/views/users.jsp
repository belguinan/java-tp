        ...
        <div class="bg-white d-flex flex-column p-4 border border-light rounded-6 shadow-sm my-auto w-100 mx-2" style="max-width: 600px;">
            ...
            <c:if test="${requestScope.users.size() > 0}">
                <div class="p-4 mt-5 rounded-6 bg-light">
                    <h2 class="h5 small mb-2">Available users:</h2>
                    <ul class="list-group border-0">
                        <c:forEach items="${requestScope.users}" var="user">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <div class="ms-2 me-auto">
                                    <div>
                                        <span class="text-muted">Name:</span> <c:out value="${user.getName()}" />
                                    </div>
                                    <span class="text-muted">Email:</span> <c:out value="${user.getEmail()}" />
                                </div>
                                <div>
                                    <form method="POST">
                                        ...
                                        <input name="_id" type="hidden" value="${user.getId()}" />
                                        <button type="submit" onclick="return confirm('Are you sure?')" class="btn btn-sm p-0">
                                            <img src="${pageContext.request.contextPath}/images/trash.svg" width="25" height="25"/>
                                        </button>
                                    </form>
                                </div>
                            </li>
